...
function hasUnsafeTargetMarkers(target) {
  ...
  const hasNonFixedParenthetical =
    /[（(][^）)]*(?:译者注|备注|说明|解释|待确认|待定|暂定|或译|又称|也称|中的|岗位|职务|职责|含义|意思|用于|负责|指的是)[^）)]*[）)]/.test(
      target,
    );
...
function validateTarget(record, location, required) {
  if (record.target === undefined && !required) {
    return [];
  }
  if (!isNonEmptyString(record.target)) {
    return [`${location}: 定稿译法必须是非空字符串`];
  }
  return hasUnsafeTargetMarkers(record.target)
    ? [`${location}: 定稿译法必须是单一且可直接替换的表达`]
    : [];
}

function validateRationale(record, location, required) {
  if (record.rationale === undefined && !required) {
    return [];
  }
  return isNonEmptyString(record.rationale)
    ? []
    : [`${location}: 取舍理由必须是非空字符串`];
}

function validateCompetitionReview(record, location, required) {
  if (record.competitionReview === undefined && !required) {
    return [];
  }
  if (!isObject(record.competitionReview)) {
    return [`${location}: 竞争译法审查必须是对象`];
  }

  const diagnostics = [];
  const { alternatives, required: hasCompetition } = record.competitionReview;
  if (typeof hasCompetition !== "boolean") {
...
function validateMismatchRiskReview(record, location, required) {
  if (record.mismatchRiskReview === undefined && !required) {
    return [];
  }
  if (!isObject(record.mismatchRiskReview)) {
    return [`${location}: 误匹配审查必须是对象`];
  }
...
function validateReviewedAt(record, location, required) {
  if (record.reviewedAt === undefined && !required) {
    return [];
  }
  return isValidDate(record.reviewedAt)
    ? []
    : [`${location}: 复查日期必须是有效的 YYYY-MM-DD 日期`];
}

function validateCandidateRecord(record, location) {
  return [
    ...validateTarget(record, location, false),
    ...validateRationale(record, location, false),
    ...validateCompetitionReview(record, location, false),
    ...validateMismatchRiskReview(record, location, false),
    ...validateReviewedAt(record, location, false),
  ];
}

function validateAcceptedRecord(record, location) {
  const diagnostics = [...validateTarget(record, location, true)];
  ...
  diagnostics.push(
    ...validateRationale(record, location, true),
    ...validateCompetitionReview(record, location, true),
    ...validateMismatchRiskReview(record, location, true),
    ...validateReviewedAt(record, location, true),
  );
