...
const concreteHostPattern =
  /^(?:\*\.)?(?:[A-Za-z0-9](?:[A-Za-z0-9-]*[A-Za-z0-9])?\.)+[A-Za-z0-9](?:[A-Za-z0-9-]*[A-Za-z0-9])?$/;

function hasConcreteHost(matchRule) {
  const schemeSeparator = matchRule.indexOf("://");
  const ruleAfterScheme =
    schemeSeparator === -1
      ? matchRule
      : matchRule.slice(schemeSeparator + "://".length);
  const host = ruleAfterScheme.split("/", 1)[0];
  return concreteHostPattern.test(host);
}
...
export function validateAnimeMeta(meta, metaPath) {
  ...
  const restrictedMatches = new Map();
  if (Array.isArray(meta.matches)) {
    for (const [index, matchRule] of meta.matches.entries()) {
      ...
      const normalizedRule = matchRule.trim();
      if (!hasConcreteHost(normalizedRule)) {
        diagnostics.push(
          `meta.matches[${index}] (${normalizedRule}): 首版不允许使用全局匹配`,
        );
        continue;
      }

      const existingIndex = restrictedMatches.get(normalizedRule);
      if (existingIndex !== undefined) {
        diagnostics.push(
          `meta.matches[${index}] (${normalizedRule}): 与第 ${existingIndex} 条站点匹配规则重复`,
        );
        continue;
      }
      restrictedMatches.set(normalizedRule, index);
    }
  }
  const restrictedMatchCount = restrictedMatches.size;
